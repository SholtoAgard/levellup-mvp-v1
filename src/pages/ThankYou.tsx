...
const ThankYou = () => {
  ...
  useEffect(() => {
    ...
    const getAssetUrls = async () => {
      ...
      const {
        data: photoExists
      } = await supabase.storage.from('assets').list('', {
        search: photoPath
      });
      console.log('Checking if photo exists:', photoExists);
      const {
        data: photoData
      } = await supabase.storage.from('assets').getPublicUrl(photoPath);
      const {
        data: scriptData
      } = await supabase.storage.from('assets').getPublicUrl(scriptPath);
